"""
clerk_scraper.py  v13 — reverted to working API core + current pipeline compatibility

Source basis:
- Restores the confirmed-working search flow from uploaded v8
- Keeps compatibility with current fetch.py

What changed:
- class name is MaricopaClerkScraper
- adds scrape() wrapper
- accepts unused kwargs
- creates grouped_output/pdfs/.keep for workflow stability
"""
...
class MaricopaClerkScraper:
    ...
    async def run(self) -> list[dict]:
        ...
        for lead_key in self.lead_types:
            ...
            if not doc_code:
                log.warning(f"No mapped document code for {lead_key}")
                continue
