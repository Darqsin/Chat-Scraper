...
def is_valid_property_address(text: str) -> bool:
    text = re.sub(r"\s+", " ", text).strip()
    upper = text.upper()

    bad_terms = [
        "LLC", "ESQ", "LAW", "ATTORNEY", "TRUSTEE", "HIMMELSTEIN", "ADKINS",
    ...
    return bool(
        re.search(
            r"\d{2,6}\s+[A-Z0-9 .'\-]+(?:ST|STREET|AVE|AVENUE|RD|ROAD|DR|DRIVE|LN|LANE|CT|COURT|PL|PLACE|BLVD|BOULEVARD|WAY)\b.*\b(AZ|ARIZONA)\b.*\d{5}",
            text,
            re.I,
        )
    )
...
def _best_address(candidates: list[str], property_mode: bool = True) -> dict[str, str]:
    ...
    for candidate in candidates:
        ...
        if property_mode and not is_valid_property_address(candidate):
            continue
...
def _parse_us_address(candidate: str) -> dict[str, str]:
    try:
        ...
        return {
            "address": candidate if _looks_like_address(candidate) else "",
            "city": "",
            "state": "",
            "zip": "",
        }
